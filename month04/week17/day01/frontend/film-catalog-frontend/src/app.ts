...
async function fetchFilms(page = 1, size = 20): Promise<Film[]>{
    const res = await fetch(`${API_BASE}/api/films?page=${page}&size=${size}`);
    if (!res.ok) throw new Error (`HTTP ${res.status}`);
    return res.json();
}

async function searchFilms(title:string):Promise<Film[]>{
...
getEl("btn-search").addEventListener("click", async () => {
    const input = getEl("input-search") as HTMLInputElement;
