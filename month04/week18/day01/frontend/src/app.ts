...
sortSelect.addEventListener("change", () => {
    const criteria = sortSelect.value as keyof Film;
    const ordered = database.sort(criteria);
    actionHistory.push(
        `SORTED: Workspace order re-indexed around target: "${criteria}"`,
    );

    // Instead of completely tracking state mutation, force-swap display slice temporarily
    tableBody.innerHTML = ordered
        .map(
            (f) =>
                `<tr>
            <td><strong>#${f.film_id}</strong></td>
...
// App Startup Paint Initialization
actionHistory.push("SYSTEM INIT: Data Storage Registries Online");
updateUI();